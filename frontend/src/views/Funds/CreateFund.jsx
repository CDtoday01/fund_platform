...
import { createFund, checkNameExists } from "../../services/FundService"; // Add a service function for name check
...
const CreateFund = () => {
    ...
    const [Fund_duration, setFundDuration] = useState("");
    ...
    useEffect(() => {
        const fetchDefaults = async () => {
            try {
                const response = await axiosInstance.get("/funds/defaults/");
                const defaults = response.data;
                const start_date = utcToLocalISO(defaults.announcing_start_date)
                setType(defaults.fund_type || "");
    ...
    useEffect(() => {
        const fetchSubcategories = async () => {
            try {
                const response = await axiosInstance.get("/funds/fund-types/");
    ...
    const handleNameBlur = async () => {
        if (name.trim() !== "") {
            try {
                ...
                console.error("Error checking Fund name existence:", error);
            }
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await createFund(newFund);
            alert("Fund added successfully");
            setErrors({});
            navigate(`/funds/${response.data.id}`);
        } catch (error) {
            if (error.response && error.response.data) {
                setErrors(error.response.data);
                console.error("Error adding Fund:", error.response.data);
                alert("Failed to add Fund. Please fix the errors and try again.");
            } else {
    ...
    return (
        <>
            <form onSubmit={handleSubmit}>
                ...
                <div>
                    ...
                    <input
                        ...
                        required
                        style={{ borderColor: errors.Fund_duration ? "red" : "" }}
                        disabled={loading}
                        placeholder="3~36"
                    />
                    {errors.Fund_duration && <span style={{ color: "red" }}>{errors.Fund_duration}</span>}
                </div>
                <div>
                    <label>產品說明：</label>
                    <textarea
            ...
            <Link to="/user/funds">
                <button disabled={loading}>Return to funds</button>
            </Link>
        </>
    );
};
